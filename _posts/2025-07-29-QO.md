...
<script>
  ...
  const TITLE = "y = A·square(fx), A=1, f=1, duty=50%";
  const DATASET_LABEL = "Square wave";
  // --------------------------------

  // Wave parameters (adjust if needed)
  const A = 1;       // amplitude
  const f = 1;       // frequency (rad units in x)
  const duty = 0.5;  // 0..1 (e.g., 0.5 = 50%)
  const xStart = 0, xEnd = 10, step = 0.01;

  // Generate square wave samples
  const xValues = [], yValues = [];
  const T = 2*Math.PI / f; // period in x (since we use fx)
  for (let x = xStart; x <= xEnd + 1e-9; x += step) {
    const phase = ((x % T) + T) % T;    // 0..T
  ...
  new Chart("myChart", {
    ...
    options: {
      ...
      scales: {
        yAxes: [{ ticks: { suggestedMin: -1.2*A, suggestedMax: 1.2*A } }]
      }
    }
  });
</script>
