...
<script>
  // ---- Customize labels ----
  const TITLE = "Square wave on one period [0, T]";
  const DATASET_LABEL = "duty = 50%";
  // --------------------------
  ...
  // Generate one duty on a single period [0, T]
  const data = [];
  const N = 500;                // number of samples
  const step = T / N;
  for (let x = 0; x <= T + 1e-12; x += step) {
    const phase = x;            // since period is T, compare with duty*T
    const y = (phase < duty * T) ? A : -A;
    data.push({ x, y });
  }
  ...
  xAxes: [{
    ...
    ticks: {
      ...
      callback: function (value) {
        ...
        if (Math.abs(value - T) < 1e-9) return 'T';
        return '';
      }
    },
    scaleLabel: {
      display: true,
      labelString: 'x (0 to T)'   // <- X axis label here
    },
    gridLines: { drawTicks: false }
  }],
  yAxes: [{
    ticks: { suggestedMin: -1.2*A, suggestedMax: 1.2*A },
    scaleLabel: {
      display: false              // <- No Y axis label
    }
  }]
}
    }
