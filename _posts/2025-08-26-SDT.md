...
dimana $\alpha$ dan $\beta$ merupakan bilangan kompleks $\mathbb{C}$.
...
$$
\begin{align*}
\bra{\lambda_i} \hat{A} \ket{\lambda_i} &= \bra{\lambda_i} \hat{A}^\dagger \ket{\lambda_i}\\
\lambda_i = \lambda_i^*
\end{align*}
$$
...
$$
\begin{align*}
\bra{\lambda_i} \hat{A} \ket{\lambda_j} &= \bra{\lambda_i} \hat{A} \ket{\lambda_j} \\
\lambda_j \braket{\lambda_i|\lambda_j} &= \lambda_i \braket{\lambda_i|\lambda_j} \\
\braket{\lambda_i|\lambda_j} (\lambda_j - \lambda_i) &= 0
\end{align*}
$$
...
dimana $\imath = \sqrt{-1}$, $\hbar = h/2\pi$ adalah konstanta Planck tereduksi, dan $\hat{H}$ adalah Hamiltonian yang merupakan operator dari energi dari sistem. Operator energi ini memenuhi persamaan

$$
\hat{H} \ket{\psi_i} = E_i \ket{\psi_i},
$$

dimana $E_i$ adalah energi, nilai eigen (eigenvalue), dari state eigen (eigenstate) tingkat ke-$i$. Nilai eigen ini diasumsikan merupakan bilangan real $\mathbb{R}$. Untuk memenuhi asumsi ini, Hamiltonian dapat diasumsikan sebagai sebuah operator yang Hermitian yang diekspresikan dengan persamaan

$$
\hat{H} = \hat{H}^\dagger,
$$

dimana $\hat{H}^\dagger$ merupakan operator adjungat (adjoint operator) yang didefinisikan melalui hasil kali dalam sebagai

$$
\bra{\phi} \hat{H} \ket{\psi} = \bra{\phi} \hat{H}^\dagger \ket{\psi}.
$$
